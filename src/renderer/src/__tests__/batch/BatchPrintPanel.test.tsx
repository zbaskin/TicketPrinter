...
describe('BatchPrintPanel', () => {
  ...
  it('shows "No data loaded" message when rows is empty', () => {
    render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
  ...
  it('has a hidden file input that accepts CSV and JSON', () => {
    const { container } = render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
  ...
  it('shows done status badge in green for done rows', () => {
    mockBatchState.rows = [{ id: 0, data: { name: 'Alice' }, status: 'done' }]
    render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
  ...
  it('shows error status badge in red for error rows', () => {
    mockBatchState.rows = [{ id: 0, data: { name: 'Alice' }, status: 'error', error: 'Paper jam' }]
    render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
  ...
  it('shows printing status badge in yellow for printing rows', () => {
    mockBatchState.rows = [{ id: 0, data: { name: 'Alice' }, status: 'printing' }]
    render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
  ...
  it('"Print All" is disabled when connection is null', () => {
    mockBatchState.rows = makePendingRows(2)
    render(<BatchPrintPanel document={sampleDoc} connection={null} />)
    expect(screen.getByRole('button', { name: /print all/i })).toBeDisabled()
  })
  ...
  it('"Print All" is enabled when rows present, connection present, not running', () => {
    mockBatchState.rows = makePendingRows(2)
    render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
    expect(screen.getByRole('button', { name: /print all/i })).not.toBeDisabled()
  })

  it('"Print All" calls startPrint with doc and connection when clicked', () => {
    mockBatchState.rows = makePendingRows(1)
    render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
    fireEvent.click(screen.getByRole('button', { name: /print all/i }))
    expect(mockStartPrint).toHaveBeenCalledWith(sampleDoc, usbConnection)
  })
  ...
  it('shows parse error inline when CSV is malformed', async () => {
    const csvContent = 'name,seat'
    const { container } = render(<BatchPrintPanel document={sampleDoc} connection={usbConnection} />)
    const fileInput = container.querySelector('input[type="file"]') as HTMLInputElement
    const file = new File([csvContent], 'data.csv', { type: 'text/csv' })
